...
export default function Speedtest({ place, onComplete }: SpeedtestProps) {
  ...
  return (
    <div className="bg-white rounded-lg p-6 shadow-lg">
      <h3 className="text-lg font-semibold mb-4">Speed Test: {place.name}</h3>

      {status === 'idle' && (
        <div className="text-center">
          <p className="text-gray-600 mb-4">Test your internet speed at this location</p>
          <button
            onClick={startTest}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-green-50 p-4 rounded-lg">
              ...
              <p className="text-2xl font-bold text-green-800">{formatSpeed(results.download)}</p>
            </div>
            <div className="bg-blue-50 p-4 rounded-lg">
              <p className="text-sm text-blue-700">Upload</p>
              <p className="text-2xl font-bold text-blue-800">{formatSpeed(results.upload)}</p>
            </div>
            <div className="bg-yellow-50 p-4 rounded-lg">
              <p className="text-sm text-yellow-700">Latency</p>
              <p className="text-2xl font-bold text-yellow-800">{results.latency.toFixed(0)} ms</p>
            </div>
            <div className="bg-purple-50 p-4 rounded-lg">
              <p className="text-sm text-purple-700">Jitter</p>
              <p className="text-2xl font-bold text-purple-800">{results.jitter.toFixed(1)} ms</p>
            </div>
          </div>
          <p className="text-center text-green-600 font-medium">Test saved successfully!</p>
        </div>
      )}
